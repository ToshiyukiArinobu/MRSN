...
namespace KyoeiSystem.Application.WCFService
{
    ...
    public class S12 : IS12 {
        ...
        /// <returns>S12_YOSG_Member</returns>
        public List<S12_YOSG_Member> GetData(int p得意先ID, int p集計年月, int p回数)
        {
            using (TRAC3Entities context = new TRAC3Entities(CommonData.TRAC3_GetConnectionString()))
            {
                context.Connection.Open();

                var ret = (from s11 in context.S12_YOSG
                           from m01 in context.M01_TOK.Where(m01 => m01.得意先KEY == s11.支払先KEY)
                           where (s11.支払先KEY == (from tok in context.M01_TOK where tok.得意先ID == p得意先ID select tok.得意先KEY).FirstOrDefault()
                           && s11.集計年月 == p集計年月 && s11.回数 == p回数 )
                           select new S12_YOSG_Member
                           {
                               支払先KEY = m01.得意先ID,
                               集計年月 = s11.集計年月,
                               回数 = s11.回数,
        ...
        /// S12_YOSGの新規追加
        /// </summary>
        /// <param name="s11tokｇ">S12_YOSG_Member</param>
        public void Insert(S12_YOSG_Member s11tokｇ)
        {
            using (TRAC3Entities context = new TRAC3Entities(CommonData.TRAC3_GetConnectionString()))
            {
                context.Connection.Open();
        ...
        /// </summary>
        /// <returns></returns>
        /// <param name="s11tokｇ">S12_toks__Member</param>
        public List<S12_YOSG_Member_Preview_csv> GetSearchListData(string p得意先IDFrom, string p得意先IDTo, string p処理年月From, string p処理年月To, string p回数From, string p回数To, int[] i得意先List)
        {
            using (TRAC3Entities context = new TRAC3Entities(CommonData.TRAC3_GetConnectionString()))
            {
                context.Connection.Open();

                var query = (from s11 in context.S12_YOSG
                             from m01 in context.M01_TOK.Where(m01 => m01.得意先KEY == s11.支払先KEY)
                             //where s11.削除日付 == null

                             select new S12_YOSG_Member_Preview_csv
                           {
                               支払先KEY = m01.得意先ID,
                               得意先名 = m01.略称名,
                               集計年月 = s11.集計年月,
                ...
                if (i得意先List.Length > 0)
                {
                    ...
                    query = query.Union(from s11 in context.S12_YOSG
                                        from m01 in context.M01_TOK.Where(m01 => m01.得意先KEY == s11.支払先KEY)
