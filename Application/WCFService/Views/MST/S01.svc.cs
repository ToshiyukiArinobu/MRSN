...
namespace KyoeiSystem.Application.WCFService
{
    ...
    public class S01 : IS01 {
        ...
        /// <returns>S01_TOKS_Member</returns>
        public List<S01_TOKS_Member> GetData(int p得意先ID, int p集計年月, int p回数)
        {
            using (TRAC3Entities context = new TRAC3Entities(CommonData.TRAC3_GetConnectionString()))
            {
                context.Connection.Open();

                var ret = (from s01 in context.S01_TOKS
                           from m01 in context.M01_TOK.Where(m01 => m01.得意先KEY == s01.得意先KEY)
                           where (s01.得意先KEY == (from tok in context.M01_TOK where tok.得意先ID == p得意先ID select tok.得意先KEY).FirstOrDefault()
                           && s01.集計年月 == p集計年月 && s01.回数 == p回数 )
                           select new S01_TOKS_Member
                           {
							   得意先KEY = m01.得意先ID,
                               集計年月 = s01.集計年月,
                               回数 = s01.回数,
