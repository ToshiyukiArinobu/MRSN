...
namespace KyoeiSystem.Application.WCFService
{
    ...
    public class TKS18010
    {
        ...
        /// </summary>
        [DataContract]
        public class TKS18010_Member_day
        {
        ...
        public class TKS18010_Member2
        {
            [DataMember]
            public DateTime? 日付 { get; set; }
            [DataMember]
            public int 部門ID1 { get; set; }
            [DataMember]
            public string 部門名1 { get; set; }
            [DataMember]
            ...
            [DataMember]
            public int 部門ID2 { get; set; }
            [DataMember]
            public string 部門名2 { get; set; }
            [DataMember]
            ...
            [DataMember]
            public int 部門ID3 { get; set; }
            [DataMember]
            public string 部門名3 { get; set; }
            [DataMember]
        ...
        public List<TKS18010_Member2> TKS18010_GetData(string p部門From, string p部門To, int?[] p部門List, string s部門List, string p作成締日, string p作成年, string p作成月, DateTime p集計期間From, DateTime p集計期間To)
        {
...
//                List<TKS18010_Member1> retList = new List<TKS18010_Member1>();
                ...
                //{
                    ...
                    var query = (from rday in retList_day
                                 from m71 in context.M71_BUM
                                 join m16 in context.M16_BYSN.Where(q => q.年月 == p作成年月) on m71.自社部門ID equals m16.部門ID into m16Group
								 join t01 in context.T01_TRN.Where(t01 => ((t01.入力区分 != 3) || (t01.入力区分 == 3 && t01.明細行 == 1))) on new { a = rday.日付, b = m71.自社部門ID } equals new { a = t01.請求日付, b = t01.自社部門ID } into t01Group
                                 //from t01g in t01Group.DefaultIfEmpty()
                                 orderby m71.自社部門ID, rday.日付
                                 select new TKS18010_Member1
                                 {
                                     日付 = rday.日付,
                                     部門ID = m71.自社部門ID,
                                     部門名 = m71.自社部門名,
                                     日計 = (t01Group.Sum(q => q.売上金額) == null ? 0 : t01Group.Sum(q => q.売上金額))
                                      ...
                                      予算 = m16Group.Select(q => q.売上予算).FirstOrDefault(),
                                      //コードFrom = p部門From,
                                      //コードTo = p部門To,
                                      //ピックアップ指定 = s部門List,
                                      //開始日付 = p集計期間From,
                    ...
                    var bumlet = (from q in query
                                  group q by q.部門ID into qGroup
                                  select new TKS18010_Let 
                                  {
                                      部門ID = qGroup.Key,
                    ...
                    query = (from q in query
                            let bb = from b in bumlet where b.合計 != 0 select b.部門ID
                            where bb.Contains(q.部門ID)
                            select  new TKS18010_Member1
                            {
                                日付 = q.日付,
                                部門ID = q.部門ID,
                        ...
                        var query2 = (from rday in retList_day
                                      ...
                                      select new TKS18010_Member1
                                      {
                                          ...
                                          日計 = (t01Group.Sum(q => q.売上金額) == null ? 0 : t01Group.Sum(q => q.売上金額))
                                             + (t01Group.Sum(q => q.請求割増１) == null ? 0 : t01Group.Sum(q => q.請求割増１))
                                             + (t01Group.Sum(q => q.請求割増２) == null ? 0 : t01Group.Sum(q => q.請求割増２)),
                                          予算 = context.M16_BYSN.Where(q => q.年月 == p作成年月).Any() == false ? 0 : context.M16_BYSN.Where(q => q.年月 == p作成年月).Sum(q => q.売上予算),
                                          //コードFrom = p部門From,
                                          //コードTo = p部門To,
                                          //ピックアップ指定 = s部門List,
                                          //開始日付 = p集計期間From,
                    ...
                    if (!string.IsNullOrEmpty(p部門From))
                    {
                        ...
                        query = query.Where(c => c.部門ID >= i部門From);
                    }
                    ...
                    if (!string.IsNullOrEmpty(p部門To))
                    {
                        ...
                        query = query.Where(c => c.部門ID <= i部門TO);
                    }
                    ...
                    if (p部門List.Length > 0)
                    {
                        ...
                        {
                            query = query3.Where(q => p部門List.Contains(q.部門ID));
                        }
                        else
                        {
                            query = query.Union(query3.Where(q => p部門List.Contains(q.部門ID)));
                        }
                    }
                    query = query.Distinct();


                    query3 = query.Union(query2).OrderBy(q => q.部門ID).ThenBy(q => q.日付);

                    //var query3 = query.OrderBy(q => q.部門ID).ThenBy(q => q.日付);
                    ...
                    var bumid =  (from que in query4
                                 group que by new {que.部門ID} into queGroup
                                 select new TKS18010_Member1()
                                 {
                                     部門ID = queGroup.Key.部門ID,
                                 }).AsQueryable();

                    List<TKS18010_Member2> retList = new List<TKS18010_Member2>();
                    int cnt = 0;
                    ...
                    foreach(var id in bumid)
                    {
                        ...
                        if (cnt >= 3)
                        {
                            var q1 = query4.Where(q => q.部門ID == bumid1);
                            var q2 = query4.Where(q => q.部門ID == bumid2);
                            var q3 = query4.Where(q => q.部門ID == bumid3);
                            var q4 = query4.Where(q => q.部門ID == bumid4);

                            foreach (var row in q1)
                            {
                                
                                TKS18010_Member2 list = new TKS18010_Member2()
                                {
                                    ...
                                    部門ID2 = q2.Where(q => q.日付 == row.日付).Select(q => q.部門ID).FirstOrDefault(),
                                    部門名2 = q2.Where(q => q.日付 == row.日付).Select(q => q.部門名).FirstOrDefault(),
                                    ...
                                    部門ID3 = q3.Where(q => q.日付 == row.日付).Select(q => q.部門ID).FirstOrDefault(),
                                    部門名3 = q3.Where(q => q.日付 == row.日付).Select(q => q.部門名).FirstOrDefault(),
        ...
        public List<TKS18010_Member1> TKS18010_GetData_CSV(string p部門From, string p部門To, int?[] p部門List, string s部門List, string p作成締日, string p作成年, string p作成月, DateTime p集計期間From, DateTime p集計期間To)
        {
            using (TRAC3Entities context = new TRAC3Entities(CommonData.TRAC3_GetConnectionString()))
            {
                try
