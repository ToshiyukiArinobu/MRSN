...
namespace KyoeiSystem.Application.WCFService
{
    ...
    public class TKS02012
    {
            ...
            using (TRAC3Entities context = new TRAC3Entities(CommonData.TRAC3_GetConnectionString()))
            {
                ...
                try
                {
                    ...
                    try
                    {
                        ...
                        {
                            // 販社・通常売掛分岐処理
                            branchAccountsRec(context, myCompany, targetStDate, targetEdDate, tok.取引先コード, tok.枝番, userId, outputPeriod);
                        }
        ...
        public void branchAccountsRec(TRAC3Entities context, int company, DateTime targetStDate, DateTime targetEdDate, int code, int eda, int userId, int outputPeriod)
        {
            ...
            {
                // 売掛情報の登録（販社）
                setAccountsRecHan(context, company, tokdata.JIS.自社コード, targetStDate, targetEdDate, code, eda, userId, mstZei, outputPeriod);
            }
            else
            {
                // 売掛情報の登録
                setAccountsRec(context, company, code, eda, targetStDate, targetEdDate, userId, mstZei, outputPeriod);
            }
        ...
        /// <param name="userId">ログインユーザID</param>
        private void setAccountsRec(TRAC3Entities context, int company, int code, int eda, DateTime targetStDate, DateTime? targetEdDate, int userId, M73_ZEI mstZei, int outputPeriod)
        {

            // 売上情報取得
            List<S08_URIKAKE_Extension> uriList = getUriInfo(context, company, code, eda, targetStDate, targetEdDate, userId, mstZei);
            ...
            // 前月残高の再設定
            S08_URIKAKE_Extension befData = getLastAccountsRec(context, company, targetStDate, code, eda, outputPeriod);
        ...
        /// <param name="eda">得意先枝番</param>
        /// <param name="userId">ログインユーザID</param>
        private void setAccountsRecHan(TRAC3Entities context, int myCompanyCode, int salesCompanyCode, DateTime targetStDate, DateTime targetEdDate, int? code, int? eda, int userId, M73_ZEI mstZei, int outputPeriod)
        {
            ...
            // 売上情報取得(販社)
            List<S08_URIKAKE_Extension> uriHanList = getHanUriInfo(context, myCompanyCode, salesCompanyCode, targetStDate, targetEdDate, userId, mstZei);
            ...
            // 前回売掛情報取得し結合
            S08_URIKAKE_Extension befData = getLastAccountsRec(context, myCompanyCode, targetStDate, code, eda, outputPeriod);
        ...
        public S08_URIKAKE_Extension getLastAccountsRec(TRAC3Entities context, int company, DateTime targetStDate, int? code, int? eda, int outputPeriod)
        {
            ...
            if (outputPeriod == (int)CommonConstants.台帳出力期間.月末締 ||  tok.Ｔ締日 == 31)
            {
                var befAccountsRec =
                    context.S08_URIKAKE
            ...
            else
            {
                ...
                ret.伝票番号 = 0;
                ret.行番号 = 0;
                ret.前月繰越 = befAccountsRec == null ? 0 : (int)befAccountsRec.当月請求額;
                ret.残高 = befAccountsRec == null ? 0 : (int)befAccountsRec.当月請求額;
                return ret;
            }
        }
        ...
        private DataTable getData(TRAC3Entities context, int company, DateTime targetStDate, DateTime targetEdDate, int? customerCode, int? customerEda)
        {
            ...
            var UrikakeQuery = context.S08_URIKAKE
                    ...
                    .Select(x => new PrintMember
                    {
                        ...
                        得意先消費税区分 = x.TOK.Ｔ消費税区分,
                        入金金額 = x.URIKAKE.入金額,
                        前月繰越 = x.URIKAKE.前月繰越,
                        残高 = x.URIKAKE.残高,
                    }).OrderBy(c => c.自社コード).ThenBy(c => c.得意先コード).ThenBy(c => c.得意先枝番).ThenBy(c => c.日付)
                        .ThenBy(c => c.伝票番号).ThenBy(c => c.行番号);
        ...
        /// <param name="userId">ログインユーザID</param>
        /// <returns></returns>
        public List<S08_URIKAKE_Extension> getUriInfo(TRAC3Entities context, int company, int code, int eda, DateTime? targetStDate, DateTime? targetEdDate, int userId,M73_ZEI mstZei)
        {
                    ...
                    .Select(x => new S08_URIKAKE_Extension
                        {
                            ...
                            通常税率消費税 = x.TOK.Ｔ消費税区分 == (int)CommonConstants.消費税区分.ID01_一括 ?  0 : ( x.URHD.売上区分 < (int)CommonConstants.売上区分.通常売上返品 ?
                                                                                                                           x.URHD.通常税率消費税 ?? 0 : (x.URHD.通常税率消費税 ?? 0) * -1),
                            軽減税率消費税 = x.TOK.Ｔ消費税区分 == (int)CommonConstants.消費税区分.ID01_一括 ? 0 : (x.URHD.売上区分 < (int)CommonConstants.売上区分.通常売上返品 ?
            ...
            {
                List<S08_URIKAKE_Extension> zei = (from x in urList
                                                   group x by new { x.自社コード, x.得意先コード, x.得意先枝番 } into sumgrp
                                                   select new S08_URIKAKE_Extension
                                                   {
        ...
        /// <param name="userId">ログインユーザID</param>
        /// <returns></returns>
        public List<S08_URIKAKE_Extension> getHanUriInfo(TRAC3Entities context, int myCompanyCode, int salesCompanyCode, DateTime? targetStDate, DateTime? targetEdDate, int userId, M73_ZEI mstZei)
        {
