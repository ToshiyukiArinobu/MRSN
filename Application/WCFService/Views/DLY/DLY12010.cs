...
namespace KyoeiSystem.Application.WCFService
{
    ...
    public class DLY12010 : BaseService
    {
        ...
        public DataSet GetData(string companyCode, string slipNumber, int pageParam, int userId)
        {
            ...
            List<T02_URHD_Search_Extension> hdList = getT02_URHD_Extension(companyCode, targetSlipNum, userId);

            List<T02_URDTL_Search_Extension> dtlList = getT02_URDTL_Extension(companyCode, targetSlipNum);
            //List<T02_URDTL_Search_Extension> dtlList = getT02_URDTL_Extension(companyCode, targetSlipNum, userId);
        ...
        /// <param name="slipNumber">伝票番号</param>
        /// <returns></returns>
        private List<T02_URHD_Search_Extension> getT02_URHD_Extension(string companyCode, string slipNumber, int userId)
        {
            using (TRAC3Entities context = new TRAC3Entities(CommonData.TRAC3_GetConnectionString()))
            {
                context.Connection.Open();
                ...
                {
                    var result =(
                        from t02 in context.T02_URHD
                        join t02_hd_han in context.T02_URHD_HAN on t02.伝票番号 equals t02_hd_han.伝票番号
                        join t02_dtl_han in context.T02_URDTL_HAN on t02.伝票番号 equals t02_dtl_han.伝票番号
                        join t03_hd_han in context.T03_SRHD_HAN on t02.伝票番号 equals t03_hd_han.伝票番号
                        join t03_dtl_han in context.T03_SRDTL_HAN on t02.伝票番号 equals t03_dtl_han.伝票番号
                        from m72 in context.M72_TNT.Where(w => w.担当者ID == userId)
