	...
	public class DLY02010
	{
		...
		public int? GetMeisaiNo(int pMeisaiNo, int vector)
		{
			using (TRAC3Entities context = new TRAC3Entities(CommonData.TRAC3_GetConnectionString()))
			{
				...
				case 1:		// ひとつ前
					No = (from t01 in context.T01_TRN
                          where t01.入力区分 == 3 && (t01.明細番号 < pMeisaiNo)
        ...
        public int GetMaxMeisaiCount()
        {
            ...
            {
                context.Connection.Open();
                int query = (from t01 in context.T01_TRN where t01.入力区分 == 3 select t01.明細番号).Distinct().Count();
                return query;
            }
        }
		...
		public List<DLY02010_RIREKI> TRN_RIREKI(int? tantoID, int? tokID, int? 表示順)
		{
			using (TRAC3Entities context = new TRAC3Entities(CommonData.TRAC3_GetConnectionString()))
			{
				context.Connection.Open();

				var ret = (from t01 in context.T01_TRN.Where(c => ((c.入力区分 == 3 && c.明細行 == 1)) && (tantoID == null || tantoID == c.入力者ID))
				...
				List<DLY02010_RIREKI> ret2 = ret.ToList();
