                        ...
                        var dtlList =
                            context.T02_URHD
                                ...
                                .GroupJoin(context.M70_JIS.Where(w => w.削除日時 == null),
                                    ...
                                    (x, y) => new { x, y })
                                .SelectMany(z => z.y.DefaultIfEmpty(),
                                (c, d) => new { c.x.SD, c.x.HN, c.x.IR, JIS = d, GroupHinName = (c.x.HN.自社品番 == HINBAN_SONOTA ? c.x.SD.y.自社品名 : null) })      // No.389 Mod
                                .GroupBy(g => new
                                {
                                    g.SD.x.会社名コード,
                                ...
                                .Select(s => new TallyMember
                                {
                                    ...
                                    色名称 = s.Key.色名称,
                                    金額 = (long)s.Sum(m => m.SD.y.金額),
                                    数量 = (int)s.Sum(m => m.SD.y.数量)
                                });
                        #endregion
