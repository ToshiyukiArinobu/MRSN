...
namespace KyoeiSystem.Application.WCFService
{
    ...
    public class SHR02011
    {
        ...
        public DataTable AccountsPayable(Dictionary<string, string> condition)
        {
            ...
            {
                // 検索パラメータを展開
                int myCompany, createYearMonth, userId, outputPeriod;
                int? customerCode, customerEda;

                getFormParams(condition, out myCompany, out createYearMonth, out customerCode, out customerEda, out userId, out outputPeriod);
                ...
                try
                {
                    ...
                    try
                    {
                        ...
                        {
                            // 販社・通常買掛作成分岐処理
                            branchAccountsPay(context, myCompany, targetStDate, targetEdDate, tok.取引先コード, tok.枝番, userId, outputPeriod);
                        }
        ...
        public void branchAccountsPay(TRAC3Entities context, int company, DateTime targetStDate, DateTime targetEdDate, int code, int eda, int userId, int outputPeriod)
        {
            ...
            M73_ZEI mstZei = (from x in context.M73_ZEI
                              where
                              x.適用開始日付 <= targetEdDate
                              select x).OrderByDescending(c => c.適用開始日付).FirstOrDefault();

            if (tokdata.JIS != null && tokdata.JIS.自社区分 == CommonConstants.自社区分.自社.GetHashCode())
            {
                // 買掛情報の登録（販社）
                setAccountsPayHan(context, company, tokdata.JIS.自社コード, targetStDate, targetEdDate, code, eda, userId, mstZei, outputPeriod);
            }
            else
            {
                // 買掛情報の登録
                setAccountsPay(context, company, code, eda, targetStDate, targetEdDate, userId, mstZei, outputPeriod);
            }
        ...
        /// <param name="userId">ログインユーザID</param>
        private void setAccountsPay(TRAC3Entities context, int company, int code, int eda, DateTime targetStDate, DateTime? targetEdDate, int userId, M73_ZEI mstZei, int outputPeriod)
        {

            // 仕入情報取得
            List<S09_KAIKAKE_Extension> srList = getSrInfo(context, company, code, eda, targetStDate, targetEdDate, userId, mstZei);

            // 揚り情報取得
            List<S09_KAIKAKE_Extension> agrList = getAgrInfo(context, company, code, eda, targetStDate, targetEdDate, userId, mstZei);
            ...
            // 前月残高の再設定
            S09_KAIKAKE_Extension befData = getLastAccountsPay(context, company, targetStDate, code, eda, outputPeriod);
        ...
        /// <param name="eda">得意先枝番</param>
        /// <param name="userId">ログインユーザID</param>
        private void setAccountsPayHan(TRAC3Entities context, int myCompanyCode, int salesCompanyCode, DateTime targetStDate, DateTime targetEdDate, int? code, int? eda, int userId, M73_ZEI mstZei, int outputPeriod)
        {
            ...
            // 仕入情報取得(販社)
            List<S09_KAIKAKE_Extension> srHanList = getHanSrInfo(context, myCompanyCode, salesCompanyCode, targetStDate, targetEdDate, userId, mstZei);

            // 揚り情報取得
            List<S09_KAIKAKE_Extension> agrList = getAgrInfo(context, myCompanyCode, code, eda, targetStDate, targetEdDate, userId, mstZei);
            ...
            // 前回買掛情報取得し結合
            S09_KAIKAKE_Extension befData = getLastAccountsPay(context, myCompanyCode, targetStDate, code, eda,outputPeriod);
        ...
        public S09_KAIKAKE_Extension getLastAccountsPay(TRAC3Entities context, int company, DateTime targetStDate, int? code, int? eda, int outputPeriod)
        {
            ...
            else
            {
                ...
                ret.伝票番号 = 0;
                ret.行番号 = 0;
                ret.前月繰越 = befAccountsPay == null ? 0 : (int)befAccountsPay.当月支払額;
                ret.残高 = befAccountsPay == null ? 0 : (int)befAccountsPay.当月支払額;
                return ret;
            }
        }
        ...
        /// <param name="userId">ログインユーザID</param>
        /// <returns></returns>
        public List<S09_KAIKAKE_Extension> getSrInfo(TRAC3Entities context, int company, int code, int eda, DateTime? targetStDate, DateTime? targetEdDate, int userId, M73_ZEI mstZei)
        {
            ...
            .Select(x => new S09_KAIKAKE_Extension
            {
                ...
                単価 = x.SRDTL.単価,
                軽減 = x.HIN.消費税区分,
                消費税区分 = x.TOK.Ｓ支払消費税区分,
                金額 = x.SRHD.仕入区分 < (int)CommonConstants.仕入区分.返品 ? x.SRDTL.金額 : x.SRDTL.金額 * -1,
                通常税率消費税 = x.TOK.Ｓ支払消費税区分 == (int)CommonConstants.消費税区分.ID01_一括 ? 0 : (x.SRHD.仕入区分 < (int)CommonConstants.仕入区分.返品 ?
                        x.SRHD.通常税率消費税 ?? 0 : (x.SRHD.通常税率消費税 ?? 0) * -1),
                軽減税率消費税 = x.TOK.Ｓ支払消費税区分 == (int)CommonConstants.消費税区分.ID01_一括 ? 0 : (x.SRHD.仕入区分 < (int)CommonConstants.仕入区分.返品 ?
            ...
            if (urList.Any() && urList[0].消費税区分 == (int)CommonConstants.消費税区分.ID01_一括)
            {
                List<S09_KAIKAKE_Extension> zei = (from x in urList
                                                   group x by new { x.自社コード, x.得意先コード, x.得意先枝番 } into sumgrp
                                                   select new S09_KAIKAKE_Extension
                                                   {
                                                       自社コード = sumgrp.Key.自社コード,
                                                       得意先コード = sumgrp.Key.得意先コード,
                                                       得意先枝番 = sumgrp.Key.得意先枝番,
                                                       日付 = (DateTime)targetEdDate,
                                                       伝票番号 = 999999,
                                                       行番号 = 0,
                                                       品番コード = 0,
                                                       自社品名 = "消費税",       // No.389 Add,
        ...
        /// <param name="userId">ログインユーザID</param>
        /// <returns></returns>
        public List<S09_KAIKAKE_Extension> getHanSrInfo(TRAC3Entities context, int myCompanyCode, int salesCompanyCode, DateTime? targetStDate, DateTime? targetEdDate, int userId, M73_ZEI mstZei)
        {
            ...
            List<S09_KAIKAKE_Extension> srList =
                context.T03_SRHD_HAN
                    ...
                    .Select(x => new S09_KAIKAKE_Extension
                    {
                        ...
                        //        x.SRHD.軽減税率消費税 ?? 0 : (x.SRHD.軽減税率消費税 ?? 0) * -1,
                        通常税率消費税 = x.TOK.Ｓ支払消費税区分 == (int)CommonConstants.消費税区分.ID01_一括 ? 0 : x.SRHD.仕入区分 < (int)CommonConstants.売上区分.通常売上返品 ?
                                      x.SRHD.通常税率消費税 ?? 0 : (x.SRHD.通常税率消費税 ?? 0) * -1,
                        軽減税率消費税 = x.TOK.Ｓ支払消費税区分 == (int)CommonConstants.消費税区分.ID01_一括 ? 0 : x.SRHD.仕入区分 < (int)CommonConstants.売上区分.通常売上返品 ?
                                x.SRHD.軽減税率消費税 ?? 0 : (x.SRHD.軽減税率消費税 ?? 0) * -1,
                        出金額 = 0,
                        前月繰越 = 0,
                        残高 = 0,
            ...
            if (srList.Any() && srList[0].消費税区分 == (int)CommonConstants.消費税区分.ID01_一括)
            {
                List<S09_KAIKAKE_Extension> zei = (from x in srList
                                                   group x by new { x.自社コード, x.得意先コード, x.得意先枝番 } into sumgrp
                                                   select new S09_KAIKAKE_Extension
                                                   {
                                                       自社コード = sumgrp.Key.自社コード,
                                                       ...
                                                       金額 = 0,
                                                       通常税率消費税 = sumgrp.Where(c => c.軽減 == (int)CommonConstants.商品消費税区分.通常税率) == null ?
                                                           //0 : sumgrp.Where(c => c.軽減 == (int)CommonConstants.商品消費税区分.通常税率).Sum(c => c.金額),
                                                                           0 : (int)Math.Floor(sumgrp.Where(c => c.軽減 == (int)CommonConstants.商品消費税区分.通常税率).Sum(c => c.金額) * (int)mstZei.消費税率 / 100d),
                                                       軽減税率消費税 = sumgrp.Where(c => c.軽減 == (int)CommonConstants.商品消費税区分.軽減税率) == null ?
                                                           //0 : sumgrp.Where(c => c.軽減 == (int)CommonConstants.商品消費税区分.通常税率).Sum(c => c.金額),
                                                                           0 : (int)Math.Floor(sumgrp.Where(c => c.軽減 == (int)CommonConstants.商品消費税区分.軽減税率).Sum(c => c.金額) * (int)mstZei.軽減税率 / 100d),
                ...
                srList = srList.Concat(zei).ToList();
            }
        ...
        /// <param name="userId">ログインユーザID</param>
        /// <returns></returns>
        public List<S09_KAIKAKE_Extension> getAgrInfo(TRAC3Entities context, int company, int? code, int? eda, DateTime? targetStDate, DateTime? targetEdDate, int userId,M73_ZEI mstZei)
        {
            ...
            List<S09_KAIKAKE_Extension> agrList =
                context.T04_AGRHD
                    ...
                    .GroupJoin(context.M01_TOK.Where(w => w.削除日時 == null),
                        ...
                        y => new { コード = y.取引先コード, 枝番 = y.枝番 },
                        (x, y) => new { x.AGRHD, x.AGRDTL, y })
                    .SelectMany(z => z.y.DefaultIfEmpty(),
                        (a, b) => new { a.AGRHD, a.AGRDTL, TOK = b })
                    .Select(x => new S09_KAIKAKE_Extension
                    {
                        ...
                        金額 = x.AGRDTL.金額 ?? 0,
                        消費税区分 = x.TOK.Ｓ支払消費税区分,
                        通常税率消費税 = x.TOK.Ｓ支払消費税区分 == (int)CommonConstants.消費税区分.ID01_一括 ? 0 : x.AGRHD.消費税 ?? 0,
                        軽減税率消費税 = 0,
                        出金額 = 0,
                        前月繰越 = 0,
                        残高 = 0,
            ...
            if (agrList.Any() && agrList[0].消費税区分 == (int)CommonConstants.消費税区分.ID01_一括)
            {
                List<S09_KAIKAKE_Extension> zei = (from x in agrList
                                                   group x by new { x.自社コード, x.得意先コード, x.得意先枝番 } into sumgrp
                                                   select new S09_KAIKAKE_Extension
                                                   {
                                                       自社コード = sumgrp.Key.自社コード,
                ...
                agrList = agrList.Concat(zei).ToList();
            }
            ...
            out int outputPeriod)
        {
            ...
            outputPeriod = int.Parse(condition["出力期間"]);
        }
        #endregion
