...
namespace KyoeiSystem.Application.Windows.Views
{
    ...
    /// 支払締集計 フォームクラス
    /// </summary>
    public partial class SHR03010 : WindowReportBase
    {
        #region 画面設定項目
        /// <summary>
        /// ユーザ設定項目
        ...
        /// ※ 必ず public で定義する。
        /// </summary>
        public class ConfigSHR03010 : FormConfigBase
        {
            public byte[] spConfig = null;
        }

        /// ※ 必ず public で定義する。
        public ConfigSHR03010 frmcfg = null;
        ...
        private const string GET_LIST_SEARCH = "SHR03010_GetDataList";
        private const string PAYMENT_AGGREGATE = "SHR03010_PaymentAggregation";
            ...
            try
            {
                ...
                switch (message.GetMessageName())
                {
                    case GET_LIST_SEARCH:
                        if (tbl == null)
                        {
                            this.sp支払データ一覧.ItemsSource = null;
        ...
        public override void OnF9Key(object sender, KeyEventArgs e)
        {
            ...
            if (sp支払データ一覧.Rows.Count == 0)
            {
                this.ErrorMessage = "指定された条件の支払データはありません。";
                MessageBox.Show("指定された条件の支払データはありません。");
                return;
            }
            ...
            {
                ErrorMessage = "作成支払年月の内容が正しくありません。";
                MessageBox.Show("入力エラーがあります。");
                return;
            }
            ...
            if (MessageBox.Show("支払締集計処理を実行しますか？",
        ...
        private bool isListValidation()
        {
            ...
            foreach (DataRow row in SearchList.Rows)
            {
                ...
                // エラー情報をクリア
                sp支払データ一覧.Rows[rIdx].ValidationErrors.Clear();
        ...
        private void Window_Closed(object sender, EventArgs e)
        {
            //this.請求書一覧データ = null;
            SearchList = null;
            sp支払データ一覧.InputBindings.Clear();

            frmcfg.Top = this.Top;
            frmcfg.Left = this.Left;
            frmcfg.Height = this.Height;
            frmcfg.Width = this.Width;
            frmcfg.spConfig = AppCommon.SaveSpConfig(this.sp支払データ一覧);
            ucfg.SetConfigValue(frmcfg);
        ...
        {
            AppCommon.LoadSpConfig(this.sp支払データ一覧, this.spConfig);
            ScreenClear();
        ...
        private void sp支払データ一覧_CellEditEnding(object sender, SpreadCellEditEndingEventArgs e)
        {
            ...
            CellText = sp支払データ一覧.Cells[e.CellPosition.Row, e.CellPosition.Column].Text;
        }

        private void sp支払データ一覧_CellEditEnded(object sender, SpreadCellEditEndedEventArgs e)
        {
            ...
            try
            {
                ...
                string ctext = sp支払データ一覧.Cells[e.CellPosition.Row, e.CellPosition.Column].Text;
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void sp支払データ一覧_PreviewKeyDown(object sender, KeyEventArgs e)
        {
            if (e.Key == Key.Delete && !sp支払データ一覧.ActiveCell.IsEditing)
                e.Handled = true;
