...
namespace KyoeiSystem.Application.Windows.Views
{
    ...
    public partial class SHR06010 : WindowReportBase
    {
        ...
        private const string SEARCH_SHR06010_CSV = "SHR06010_GetCsvData";

        /// <summary>帳票印刷データ取得</summary>
        private const string SEARCH_SHR06010_PRT = "SHR06010_GetPrintData";

        /// <summary>帳票定義ファイル 格納パス</summary>
        private const string ReportTemplateFileName = @"Files\SHR\SHR06010.rpt";
        ...
        /// ※ 必ず public で定義する。
        /// </summary>
        public class ConfigSHR06010 : FormConfigBase
        {

        }
        /// ※ 必ず public で定義する。
        public ConfigSHR06010 frmcfg = null;
        ...
        private void RibbonWindow_Loaded(object sender, RoutedEventArgs e)
        {
            ...
            frmcfg = (ConfigSHR06010)ucfg.GetConfigValue(typeof(ConfigSHR06010));
            ccfg = (CommonConfig)ucfg.GetConfigValue(typeof(CommonConfig));

            if (frmcfg == null)
            {
                frmcfg = new ConfigSHR06010();
        ...
        public override void OnReceivedResponseData(CommunicationObject message)
        {
            try
            {
                ...
                if (data is DataTable)
                {
                    ...
                    {
                        case SEARCH_SHR06010_PRT:
                            // 検索結果取得時
                            DispPreviw(tbl);
                            break;

                        case SEARCH_SHR06010_CSV:
                            OutPutCSV(tbl);
                            break;
        ...
        private void DispPreviw(DataTable tbl)
        {
            try
            {
                ...
                string closingText = isPaymentAllDays.IsChecked == true ?
                    isPaymentAllDays.Content.ToString() : string.Format("{0}日締支払分", PaymentDay.Text);
                ...
                var parms = new List<FwPreview.ReportParameter>()
                {
                    ...
                    new FwPreview.ReportParameter(){ PNAME="支払締日", VALUE=(closingText)},
                ...
                // 第2引数　帳票ファイルPass
                // 第3以上　帳票の開始点(0で良い)
                view.MakeReport("出金予定表", ReportTemplateFileName, 0, 0, 0);
                // 帳票ファイルに送るデータ。
        ...
        private bool CheckFormValid()
        {
            ...
            if (string.IsNullOrEmpty(PaymentYearMonth.Text))
            {
                ...
                ErrorMessage = "出金年月が入力されていません。";
                return false;
            }

            if (isPaymentAllDays.IsChecked == true)
            {
                // 全出金日の状態チェック(作成出金日入力かつチェックオンはエラー)
        ...
        private Dictionary<string, string> createParamDic()
        {
            ...
            paramDic.Add("支払年月", PaymentYearMonth.Text);
            paramDic.Add("支払日", PaymentDay.Text);
            paramDic.Add("全締日", isPaymentAllDays.IsChecked.ToString());
            paramDic.Add("支払先コード", Customer.Text1 == null ? string.Empty : Customer.Text1);
            paramDic.Add("支払先枝番", Customer.Text2 == null ? string.Empty : Customer.Text2);
        ...
        #region 支払締日チェックオンのイベント
        /// <summary>
        /// 支払締日がチェックされた時のイベント処理
        /// </summary>
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void isPaymentAllDays_Checked(object sender, RoutedEventArgs e)
        {
            // 締日チェックがオンになる場合に締日テキストを初期化する
