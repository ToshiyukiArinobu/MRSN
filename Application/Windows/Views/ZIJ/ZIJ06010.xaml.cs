...
namespace KyoeiSystem.Application.Windows.Views
{
    ...
    /// <summary>
    /// 商品移動/振替入力問合せ フォームクラス
    /// </summary>
    public partial class ZIJ06010 : RibbonWindowViewBase
    {
        #region << 列挙型定義 >>
        ...
        private const string GET_DATA = "ZIJ06010_GetData";

        #region レポート定義

        // 商品在庫一覧表
        const string ReportFileName = @"Files\ZIJ\ZIJ06010.rpt";
        ...
        /// ※ 必ず public で定義する。
        /// </summary>
        public class ConfigZIJ06010 : FormConfigBase
        {
            public byte[] spConfigZIJ06010 = null;

        }

        /// ※ 必ず public で定義する。
        public ConfigZIJ06010 frmcfg = null;
...
*/
        ...
        public override void OnF8Key(object sender, KeyEventArgs e)
        {
            ...
            try
            {
                ...
                {
                    new FwRepPreview.ReportParameter() { PNAME = "移動開始日", VALUE = getReportParameterValue("移動開始日") },
                    new FwRepPreview.ReportParameter() { PNAME = "移動終了日", VALUE = getReportParameterValue("移動終了日") },
                    new FwRepPreview.ReportParameter() { PNAME = "自社品名", VALUE = getReportParameterValue("自社品名") },
                    new FwRepPreview.ReportParameter() { PNAME = "出荷元倉庫名", VALUE = getReportParameterValue("出荷元倉庫名") },
                    new FwRepPreview.ReportParameter() { PNAME = "出荷先倉庫名", VALUE = getReportParameterValue("出荷先倉庫名") },
                };

                FwRepPreview.ReportPreview view = new FwRepPreview.ReportPreview();

                DataTable 印刷データ = SearchResult.Copy();
                印刷データ.TableName = "商品移動/振替入力問合せ";
            ...
            catch (Exception ex)
            {
                ...
                appLog.Error("商品移動/振替入力問合せ一覧の印刷時に例外が発生しました。", ex);
        ...
        private void Window_Closed(object sender, EventArgs e)
        {
            if (ucfg != null)
            {
                if (frmcfg == null) { frmcfg = new ConfigZIJ06010(); }
                frmcfg.Top = this.Top;
        ...
        private void initSearchControl()
        {
            ...
            // 出荷先倉庫
            this.txtDistination.Text1 = string.Empty;
        ...
        private void setSearchContidions()
        {
            ...
            conditions.Add("移動開始日", txtMoveDatePriod.Text1);
            conditions.Add("移動終了日", txtMoveDatePriod.Text2);
            conditions.Add("自社品番", txtProduct.Text1);
            conditions.Add("出荷元倉庫", txtConsignor.Text1);
            conditions.Add("出庫先倉庫", txtDistination.Text1);

            conditions.Add("自社品名", txtProduct.Text2);
            conditions.Add("出荷元倉庫名", txtConsignor.Text2);
            conditions.Add("出荷先倉庫名", txtDistination.Text2);
