...
namespace KyoeiSystem.Application.Windows.Views
{
    ...
    /// セット品番一括修正
    /// </summary>
    public partial class MST10011 : RibbonWindowViewBase
    {
        #region << 列挙型定義 >>
        /// <summary>
        /// データグリッドの新列定義
        ...
        /// ※ 必ず public で定義する。
        /// </summary>
        public class ConfigMST20011 : FormConfigBase
        {
        ...
        /// ※ 必ず public で定義する。
        public ConfigMST20011 frmcfg = null;
        ...
        private const string MST10011_GetData = "MST10011_GetData";
        private const string MST10011_Update = "MST10011_Update";
        ...
        private string _自社品番 = string.Empty;
        public string 自社品番
        {
            get { return this._自社品番; }
            set { this._自社品番 = value; NotifyPropertyChanged(); }
        }
        ...
        private void MainWindow_Loaded(object sender, RoutedEventArgs e)
        {
            ...
            frmcfg = (ConfigMST20011)ucfg.GetConfigValue(typeof(ConfigMST20011));
            if (frmcfg == null)
            {
                frmcfg = new ConfigMST20011();
        ...
        public override void OnF9Key(object sender, KeyEventArgs e)
        {
            try
            {
                ...
                base.SendRequest(
                    new CommunicationObject(MessageType.UpdateData, MST10011_Update, new object[]{
                            ds,
                            ccfg.ユーザID,
                        }));
        ...
        private bool UpdateValidataCheck()
        {
            ...
            for (int i = 0; i < SearchResult.Rows.Count; i++)
            {
                ...
                //品番コード	int	Yes
                //自社品番	varchar(12)	
                bytenum = sjisEnc.GetByteCount(drData["セット品番"].ToString());
                if (bytenum > 12)
                {
        ...
        private void Button_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                ...
                base.SendRequest(
                    new CommunicationObject(MessageType.RequestData,MST10011_GetData,new object[]
                        {
                            自社品番
                        }));
        ...
        private void MainWindow_Closed(object sender, EventArgs e)
        {
            ...
            if (ucfg != null)
            {
                if (frmcfg == null) { frmcfg = new ConfigMST20011(); }
                frmcfg.Top = this.Top;
        ...
        private void SetData(DataTable tbl)
        {
            ...
            string preSetProduct = string.Empty;
            for (int row = 0; row <= tbl.Rows.Count - 1; row++)
            {
                ...
                //自社品番
                spGridList[iSpdRowIndex, GridColumnsMapping.セット品番.GetHashCode()].Value = tbl.Rows[row]["セット品番"].ToString();
                //色
                spGridList[iSpdRowIndex, GridColumnsMapping.色.GetHashCode()].Value = tbl.Rows[row]["色"].ToString();
                //得意先コード
                spGridList[iSpdRowIndex, GridColumnsMapping.セット品名.GetHashCode()].Value = tbl.Rows[row]["セット品名"].ToString();
                //枝番
                spGridList[iSpdRowIndex, GridColumnsMapping.構成品番.GetHashCode()].Value = tbl.Rows[row]["構成品番"].ToString();
                //得意先名
                spGridList[iSpdRowIndex, GridColumnsMapping.構成品色.GetHashCode()].Value = tbl.Rows[row]["構成品色"].ToString();
                //得意先品番
                spGridList[iSpdRowIndex, GridColumnsMapping.構成品名.GetHashCode()].Value = tbl.Rows[row]["構成品名"].ToString();
                //使用数量
                spGridList[iSpdRowIndex, GridColumnsMapping.使用数量.GetHashCode()].Value = tbl.Rows[row]["使用数量"].ToString();

                //スプレッド行インデックスインクリメント
                iSpdRowIndex = iSpdRowIndex + 1;

                if (string.IsNullOrEmpty(preSetProduct) == false)
                {
                    if (preSetProduct != (tbl.Rows[row]["セット品番"].ToString() + "_" + tbl.Rows[row]["色"].ToString()))
                    {
                        try
                        {
                            spGridList.SetBorder(new CellRange(iSpdRowIndex - 1, 0), new BorderLine(Color.FromRgb(0, 0, 0), BorderLineStyle.Double), SetBorderOptions.Top);
                            spGridList.SetBorder(new CellRange(iSpdRowIndex - 1, 1), new BorderLine(Color.FromRgb(0, 0, 0), BorderLineStyle.Double), SetBorderOptions.Top);
                            spGridList.SetBorder(new CellRange(iSpdRowIndex - 1, 2), new BorderLine(Color.FromRgb(0, 0, 0), BorderLineStyle.Double), SetBorderOptions.Top);
                            spGridList.SetBorder(new CellRange(iSpdRowIndex - 1, 3), new BorderLine(Color.FromRgb(0, 0, 0), BorderLineStyle.Double), SetBorderOptions.Top);
                            spGridList.SetBorder(new CellRange(iSpdRowIndex - 1, 4), new BorderLine(Color.FromRgb(0, 0, 0), BorderLineStyle.Double), SetBorderOptions.Top);
                            spGridList.SetBorder(new CellRange(iSpdRowIndex - 1, 5), new BorderLine(Color.FromRgb(0, 0, 0), BorderLineStyle.Double), SetBorderOptions.Top);
                            spGridList.SetBorder(new CellRange(iSpdRowIndex - 1, 6), new BorderLine(Color.FromRgb(0, 0, 0), BorderLineStyle.Double), SetBorderOptions.Top);
                        }
                        catch (Exception ex)
                        {
                            throw ex;
                ...
                preSetProduct = tbl.Rows[row]["セット品番"].ToString() + "_" + tbl.Rows[row]["色"].ToString();
        ...
        private void CSVInPut_Click(object sender, RoutedEventArgs e)
        {
            ...
            SearchResult = tbl;
            SearchResult.TableName = MST10011_GetData;
            SetData(tbl);
